...
import { UixAutocompleteService } from "./autocomplete.service";


@Directive({
    selector: '[uixautocomplete], uixautocomplete',
    standalone: true,
  })
  ...
  export class UixAutoCompleteDirective {
    ...
    constructor(private UixAutoCompleteService : UixAutocompleteService){};
        ...
        switchMap((searchTerm : string) => {
          ...
          const filteredData = this.filterStaticData(searchTerm);
            ...
            if (searchTerm === "" && Array.isArray(this.searchData)){
              ...
              this.filterDynamicData(searchTerm);
            }
            return of([]);
          }
          })
